...
if dev_cursor1.rowcount == 0:
    print('No records found for this vendor run ' + dev_query1)
else:
    print('This run already done. Quit ' + dev_query1) 
    
    
    
                    print(dev_values)
                dev_query = """INSERT INTO dbo.NM_PI_VendorRecoveries 
                        (VendorName, VendorFileName, ValidationDate, IdentifiedDate, VendorFileDate, VendorClaimNbr, EDWClaimNbr, EDW_BIDWClaimNbr, lineofbusinessname, EDW_PaidAmtActual, VendorAmt, FinalAmt)
                        VALUES (?,?,?,?,?,?,?,?,?,?,?,?)"""
